    ...
    module Taggable #:nodoc:
      ...
      module SingletonMethods
        ...
        def tags_for_user(user)
          taggable_type = "#{self}"
          query_string =  "SELECT tags.*, COUNT( * ) AS 'count' FROM tags INNER JOIN taggings ON taggings.tag_id = tags.id " +
           "WHERE taggings.user_id = #{user.id} " + 
           "AND taggings.taggable_type = '#{taggable_type}' " + 
           "GROUP BY tags.name "         
           tags = Tag.find_by_sql([ query_string  ])
           return tags
       end
      end
